...
/*/         
User Function Ctbr041()
PRIVATE titulo		:= ""
Private nomeprog	:= "CTBR041"
Private aSelFil		:= {}

If TRepInUse() 
	Alert("Entrou CTBR046R4")
	
	U_CTBR046R4("CTBR041")
	
Else
	Alert("Entrou U_CTBR041R3")
	U_CTBR041R3()
EndIf
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function Ctr041Det(oPrint,i,titulo)
	...
	If mv_par11 == 1							// Grupo Diferente - Totaliza e Quebra
		If cGrupo != GRUPO                                   
			...
			If mv_par16 = 1
				...
				ValorCTB(nGrpDeb,lin+15,1730,16,nDecimais,.F.,cPicture,"1",CONTA,.T.,oPrint, , ,lPrintZero,,lColDbCr)				
				oPrint:Line(lin,2030,lin+55,2030)   	// vertical
				ValorCTB(nGrpCrd,lin+15,2090,16,nDecimais,.F.,cPicture,"2",CONTA,.T.,oPrint, , ,lPrintZero,,lColDbCr)				
				oPrint:Line(lin,2400,lin+55,2400)   	// vertical

				If lRedStorn 
					nTotMov := nTotMov + iif(cArqTmp->NORMAL=="1",nGrpDeb - nGrpCrd,nGrpCrd - nGrpDeb)
				Else
					nTotMov := nTotMov + (nGrpCrd - nGrpDeb)
				Endif	
				If Round(NoRound(nTotMov,3),2) < 0
					ValorCTB(nTotMov,lin + 15,2420,16,nDecimais,.T.,cPicture,"1",, .T., oPrint, , ,lPrintZero,,lColDbCr)				
				ElseIf Round(NoRound(nTotMov,3),2) > 0
					ValorCTB(nTotMov,lin + 15,2420,16,nDecimais,.T.,cPicture,"2",, .T., oPrint, , ,lPrintZero,,lColDbCr)				
                EndIf

				oPrint:Line(lin,2740,lin+55,2740)   	// vertical
			Else
				oPrint:Line(lin,1850,lin+55,1850)   	// vertical
				ValorCTB(nGrpDeb,lin+5,1850,16,nDecimais,.F.,cPicture,"1",,.T.,oPrint, , ,lPrintZero,,lColDbCr)				
				oPrint:Line(lin,2250,lin+55,2250)   	// vertical
				ValorCTB(nGrpCrd,lin+5,2250,16,nDecimais,.F.,cPicture,"2",,.T.,oPrint, , ,lPrintZero,,lColDbCr)				
...
//	If lin > 2810		
	...
	If mv_par16 = 1
		...
		ValorCTB(SALDODEB,lin+15,1730,16,nDecimais,.F.,cPicture,NORMAL,CONTA,.T.,oPrint, , ,lPrintZero,,lColDbCr)				
		oPrint:Line(lin,2030,lin+50,2030)   	// vertical
		ValorCTB(SALDOCRD,lin+15,2090,16,nDecimais,.F.,cPicture,NORMAL,CONTA,.T.,oPrint, , ,lPrintZero,,lColDbCr)				
	...
	Else
		...
		ValorCTB(SALDODEB,lin+15,1850,16,nDecimais,.F.,cPicture,NORMAL,CONTA,.T.,oPrint, , ,lPrintZero,,lColDbCr)				
		oPrint:Line(lin,2250,lin+50,2250)   	// vertical
		ValorCTB(SALDOCRD,lin+15,2250,16,nDecimais,.F.,cPicture,NORMAL,CONTA,.T.,oPrint, , ,lPrintZero,,lColDbCr)				
...
If mv_par11 == 1							// Grupo Diferente - Totaliza e Quebra
	If cGrupo != GRUPO
		...
		If mv_par16 = 1
			...
			Endif	
			If Round(NoRound(nTotMov,3),2) < 0
				ValorCTB(nTotMov,lin + 15,2420,16,nDecimais,.T.,cPicture,"1",,, oPrint, , ,lPrintZero,,lColDbCr)				
			ElseIf Round(NoRound(nTotMov,3),2) > 0
				ValorCTB(nTotMov,lin + 15,2420,16,nDecimais,.T.,cPicture,"2",,, oPrint, , ,lPrintZero,,lColDbCr)				
    		EndIf
...
If mv_par16 = 1
	oPrint:Line(lin,1730,lin+55,1730)   	// vertical
	ValorCTB(nTotDeb,lin+15,1730,16,nDecimais,.F.,cPicture,"1",CONTA,.T.,oPrint, , ,lPrintZero,,lColDbCr)				
	oPrint:Line(lin,2030,lin+55,2030)   	// vertical
	ValorCTB(nTotCrd,lin+15,2090,16,nDecimais,.F.,cPicture,"2",CONTA,.T.,oPrint, , ,lPrintZero,,lColDbCr)				
	oPrint:Line(lin,2400,lin+55,2400)   	// vertical
	nTotMov := nTotMov + (nGrpCrd - nGrpDeb)
	If Round(NoRound(nTotMov,3),2) < 0
		ValorCTB(nTotMov,lin + 15,2420,16,nDecimais,.T.,cPicture,"1",,.T., oPrint, , ,lPrintZero,,lColDbCr)				
...
Else
	oPrint:Line(lin,1850,lin+55,1850)   	// vertical
	ValorCTB(nTotDeb,lin+15,1850,16,nDecimais,.F.,cPicture,"1",,.T.,oPrint, , ,lPrintZero,,lColDbCr)				
	oPrint:Line(lin,2250,lin+55,2250)   	// vertical
	ValorCTB(nTotCrd,lin+15,2250,16,nDecimais,.F.,cPicture,"2",,.T.,oPrint, , ,lPrintZero,,lColDbCr)				
	oPrint:Line(lin,2700,lin+55,2700)   	// vertical	     
Endif
oPrint:Line(lin,3150,lin+55,3150)   	// vertical
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function Ctr041Cfg(titulo)
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function CTR041Esp(oPrint)
